...
const Control = () => {
	const {
		connected,
		setConnected,
		sendCommand, //! Look ahead: sanitize error as well
		addLog,
		setPort,
		setWriter,
		setReader,
	...
	const connect = async () => {
		...
		if ("serial" in navigator) {
			...
			//! Move up towards start of block to be more readable
			addLog("Web Serial API not supported.");
		}
	};

	//! Still maintain some sort of cleanup, prone to memory leaks
	//! I'd recommend separate try/catch for each call.
